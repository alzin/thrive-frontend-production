...
export const QuizSlideEditor: React.FC<QuizSlideEditorProps> = ({
  slide,
  index,
  onUpdateContent,
}) => {
  ...
  const updateQuizContent = (updates: any) => {
  ...
  const addOption = () => {
    const newOptions = [...(quizContent.options || []), `Option ${(quizContent.options?.length || 0) + 1}`];
  ...
  const removeOption = (optionIndex: number) => {
    ...
    const newOptions = quizContent.options?.filter((_: any, i: number) => i !== optionIndex) || [];
    let updates: any = { options: newOptions };

    // Handle single choice - adjust correctAnswer if needed
    if (quizContent.type === 'single-choice') {
      if (quizContent.correctAnswer >= optionIndex && quizContent.correctAnswer > 0) {
  ...
  return (
    <Stack spacing={3}>
      ...
      <TextField
        ...
        onChange={(e) => updateQuizContent({ question: e.target.value })}
        error={!quizContent.question?.trim()}
        helperText={!quizContent.question?.trim() ? 'Question is required' : ''}
      ...
      <FormControl fullWidth>
        ...
        <Select
          ...
          onChange={(e) => {
            ...
            } else if (newType === 'multiple-choice' && quizContent.type === 'single-choice') {
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => updateQuizContent({ randomizeOptions: e.target.checked })}
                />
              }
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => updateQuizContent({ allowRetry: e.target.checked })}
                />
              }
              label="Allow retry"
            />
      ...
      <TextField
        ...
        onChange={(e) => updateQuizContent({ explanation: e.target.value })}
            ...
            <RadioGroup>
              {(quizContent.options || []).map((option: string, optionIndex: number) => (
                <FormControlLabel
                  ...
                  sx={{
                    bgcolor: quizContent.correctAnswer === optionIndex ? 'success.light' : 'transparent',
                    borderRadius: 1,
                    px: 1,
                    border: quizContent.correctAnswer === optionIndex ? '2px solid' : 'none',
            ...
            <Stack spacing={1}>
              {(quizContent.options || []).map((option: string, optionIndex: number) => (
                <FormControlLabel
                  ...
                  label={option || `Option ${optionIndex + 1}`}
                  sx={{
                    bgcolor: quizContent.correctAnswers?.includes(optionIndex) ? 'success.light' : 'transparent',
                    borderRadius: 1,
                    px: 1,
                    border: quizContent.correctAnswers?.includes(optionIndex) ? '2px solid' : 'none',
                    borderColor: 'success.main'
                  }}
                />
              ))}
